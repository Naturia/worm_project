...
class TouchSensor():
    def __init__(self, N=3, M=20, v=0, step_size=0.01): #0.5, 1.7 N up M down
        self.v = v
...
class WormNet_n:
    def __init__(self, method='rk2', step_size=1 * ms, worm_entity=None, nacl_entity=None, touch_entity=None,
        ...
        head_sources_chemi, head_targets_chemi, head_weights_chemi = self.get_head_chemical_connections(
            self.headneuron_index)
        head_sources_gap, head_targets_gap, head_weights_gap = self.get_head_gap_connections(self.headneuron_index)
        self.headsyn_chemi = self.connect_group(self.headnet, head_sources_chemi, head_targets_chemi,
                                                head_weights_chemi, neuron_chemical_syn_eqs)
        self.headsyn_gap = self.connect_group(self.headnet, head_sources_gap, head_targets_gap, head_weights_gap,
                                              neuron_gap_syn_eqs)
        ...
        vnc_sources_chemi, vnc_targets_chemi, vnc_weights_chemi = self.get_vnc_chemical_connections(
            self.vncneuron_index)
        self.vncsyn_chemi = self.connect_group(self.vncnet, vnc_sources_chemi, vnc_targets_chemi, vnc_weights_chemi,
                                               neuron_chemical_syn_eqs)
        vnc_sources_gap, vnc_targets_gap, vnc_weights_gap = self.get_vnc_gap_connections(self.vncneuron_index)
        self.vncsyn_gap = self.connect_group(self.vncnet, vnc_sources_gap, vnc_targets_gap, vnc_weights_gap,
                                             neuron_gap_syn_eqs)
        ...
        self.vncbsyn_chemi = self.connect_group(self.vncbnet, vncb_sources_chemi, vncb_targets_chemi,
                                                vncb_weights_chemi,
                                                neuron_chemical_syn_eqs)
        ...
        motor_control_sources_chemi, motor_control_targets_chemi, motor_control_weights_chemi = self.get_motor_control_chemical_connections(
            self.motor_control_index)
        self.motor_control_chemi = self.connect_group(self.motor_control_net, motor_control_sources_chemi,
                                                      motor_control_targets_chemi, motor_control_weights_chemi,
                                                      neuron_chemical_syn_eqs)
        motor_control_sources_gap, motor_control_targets_gap, motor_control_weights_gap = self.get_motor_control_gap_connections(
            self.motor_control_index)
        self.motor_control_gap = self.connect_group(self.motor_control_net, motor_control_sources_gap,
                                                    motor_control_targets_gap, motor_control_weights_gap,
                                                    neuron_gap_syn_eqs)
        ...
        if record:
            ...
            self.net.add(
                [self.headnet, self.headsyn_chemi, self.headsyn_gap, self.tailnet, self.tailsyn_chemi, self.tailsyn_gap,
                 self.vncnet, self.vncsyn_chemi, self.vncbsyn_gap, self.vncbnet, self.vncbsyn_chemi, self.vncbsyn_gap,self.body_op, self.motor_control_net,
                 self.motor_control_chemi, self.motor_control_gap, self.M, self.M_2])
        else:
            self.net.add(
                [self.headnet, self.headsyn_chemi, self.headsyn_gap, self.tailnet, self.tailsyn_chemi, self.tailsyn_gap,
                 self.vncnet, self.vncsyn_chemi, self.vncsyn_gap, self.vncbnet, self.vncbsyn_chemi, self.vncbsyn_gap, self.body_op, self.motor_control_net,
    ...
    def create_head_neuron(self, num, eqs, method):
        ...
        headnet = self.set_head_neuron_parameters(headnet, 'SMDD', c_m=self.head_parameters["SMDD_cm"],
                                                  delta=self.head_parameters["SMDD_delta"],
                                                  v=self.head_parameters["SMDD_v"])
        headnet = self.set_head_neuron_parameters(headnet, 'SMDV', c_m=self.head_parameters["SMDV_cm"],
                                                  delta=self.head_parameters["SMDV_delta"],
                                                  v=self.head_parameters["SMDV_v"])
        headnet = self.set_head_neuron_parameters(headnet, 'RMDD', c_m=self.head_parameters["RMDD_cm"],
                                                  delta=self.head_parameters["RMDD_delta"],
                                                  v=self.head_parameters["RMDD_v"])
        headnet = self.set_head_neuron_parameters(headnet, 'RMDV', c_m=self.head_parameters["RMDV_cm"],
                                                  delta=self.head_parameters["RMDV_delta"],
                                                  v=self.head_parameters["RMDV_v"])
    ...
    def create_vnc_neuron(self, num, eqs, method):
        ...
        for i in range(self.unit_num):
            ...
            vncnet = self.set_vnc_neuron_parameters(vncnet, unit_jumper + self.vncneuron_index['VB'],
                                                    c_m=self.vnc_parameters["VB_cm"],
                                                    delta=self.vnc_parameters["VB_delta"],
                                                    v=self.vnc_parameters["VB_v"])
            vncnet = self.set_vnc_neuron_parameters(vncnet, unit_jumper + self.vncneuron_index['DB'],
                                                    c_m=self.vnc_parameters["DB_cm"],
                                                    delta=self.vnc_parameters["DB_delta"],
                                                    v=self.vnc_parameters["DB_v"])
            vncnet = self.set_vnc_neuron_parameters(vncnet, unit_jumper + self.vncneuron_index['VD'],
                                                    c_m=self.vnc_parameters["VD_cm"],
                                                    delta=self.vnc_parameters["VD_delta"],
                                                    v=self.vnc_parameters["VD_v"])
            vncnet = self.set_vnc_neuron_parameters(vncnet, unit_jumper + self.vncneuron_index['DD'],
                                                    c_m=self.vnc_parameters["DD_cm"],
                                                    delta=self.vnc_parameters["DD_delta"],
                                                    v=self.vnc_parameters["DD_v"])
        # AVB
    ...
    def get_vnc_chemical_connections(self, vnc_indexs):
        ...
        for i in range(self.unit_num):
            unit_jumper = i * self.neuron_per_unit
            weights[unit_jumper + vnc_indexs['DB'], unit_jumper + vnc_indexs['VD']] = self.vnc_chemical_parameters[
                "DB_to_VD"]
            weights[unit_jumper + vnc_indexs['VB'], unit_jumper + vnc_indexs['DD']] = self.vnc_chemical_parameters[
                "VB_to_DD"]
            # weights[unit_jumper + vnc_indexs['VD'],unit_jumper + vnc_indexs['VB']] = -10
            weights[unit_jumper + vnc_indexs['DB'], unit_jumper + vnc_indexs['DD']] = self.vnc_chemical_parameters[
                "DB_to_DD"]
            weights[unit_jumper + vnc_indexs['VB'], unit_jumper + vnc_indexs['VD']] = self.vnc_chemical_parameters[
                "VB_to_VD"]
    ...
    def get_vnc_gap_connections(self, vnc_indexs):
        ...
        for i in range(self.unit_num - 1):
            pre_unit_jumper = i * self.neuron_per_unit
            post_unit_jumper = (i + 1) * self.neuron_per_unit
            weights[pre_unit_jumper + vnc_indexs['DB'], post_unit_jumper + vnc_indexs['DB']] = self.vnc_gap_parameters[
                "DB_DB"]
            weights[pre_unit_jumper + vnc_indexs['VB'], post_unit_jumper + vnc_indexs['VB']] = self.vnc_gap_parameters[
                "VB_VB"]
            weights[pre_unit_jumper + vnc_indexs['DD'], post_unit_jumper + vnc_indexs['DD']] = self.vnc_gap_parameters[
                "DD_DD"]
            weights[pre_unit_jumper + vnc_indexs['VD'], post_unit_jumper + vnc_indexs['VD']] = self.vnc_gap_parameters[
                "VD_VD"]

            weights[post_unit_jumper + vnc_indexs['DB'], pre_unit_jumper + vnc_indexs['DB']] = self.vnc_gap_parameters[
                "DB_DB"]
            weights[post_unit_jumper + vnc_indexs['VB'], pre_unit_jumper + vnc_indexs['VB']] = self.vnc_gap_parameters[
                "VB_VB"]
            weights[post_unit_jumper + vnc_indexs['DD'], pre_unit_jumper + vnc_indexs['DD']] = self.vnc_gap_parameters[
                "DD_DD"]
            weights[post_unit_jumper + vnc_indexs['VD'], pre_unit_jumper + vnc_indexs['VD']] = self.vnc_gap_parameters[
                "VD_VD"]
    ...
    def get_vncb_gap_connections(self, vncb_indexs):
        ...
        for i in range(self.unit_num - 1):
            pre_unit_jumper = i * self.neuron_per_unit
            post_unit_jumper = (i + 1) * self.neuron_per_unit
            weights[pre_unit_jumper + vncb_indexs['DA'], post_unit_jumper + vncb_indexs['DA']] = \
            self.vncb_gap_parameters[
                "DA_DA"]
            weights[pre_unit_jumper + vncb_indexs['VA'], post_unit_jumper + vncb_indexs['VA']] = \
            self.vncb_gap_parameters[
                "VA_VA"]
            weights[pre_unit_jumper + vncb_indexs['DD'], post_unit_jumper + vncb_indexs['DD']] = \
            self.vncb_gap_parameters[
                "DD_DD"]
            weights[pre_unit_jumper + vncb_indexs['VD'], post_unit_jumper + vncb_indexs['VD']] = \
            self.vncb_gap_parameters[
                "VD_VD"]

            weights[post_unit_jumper + vncb_indexs['DA'], pre_unit_jumper + vncb_indexs['DA']] = \
            self.vncb_gap_parameters[
                "DA_DA"]
            weights[post_unit_jumper + vncb_indexs['VA'], pre_unit_jumper + vncb_indexs['VA']] = \
            self.vncb_gap_parameters[
                "VA_VA"]
            weights[post_unit_jumper + vncb_indexs['DD'], pre_unit_jumper + vncb_indexs['DD']] = \
            self.vncb_gap_parameters[
                "DD_DD"]
            weights[post_unit_jumper + vncb_indexs['VD'], pre_unit_jumper + vncb_indexs['VD']] = \
            self.vncb_gap_parameters[
                "VD_VD"]
