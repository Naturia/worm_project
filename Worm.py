...
class Worm:
    def __init__(self,segment_num = 50, muscle_num = 24, step_size=0.01, parameters=None, test=False, print_info=True, shared_lists = None, gui_parameters= None, nacl_location=None, state_queue = None):
        ...
        if parameters is None:
            ...
            "head_sr_gain": 1200,
            "tail_sr_gain": 1200,
            "muscle_tau": 0.1,  #muscle resistency
            "turn_gain": 0.5,
    ...
    def step(self,neuron_net,nacl,touch):
        ...
        if self.avm_h ==0 and self.plm_h == 0:
            ...
            #     print("Omega turn")
            #     self.body.set_dorsal_activation(0, self.ms.get_dorsal_activation(0) / 2 * 3)
            #     self.body.set_ventral_activation(0, self.ms.get_ventral_activation(0) / 2 * 1.8)
            #     self.body.set_dorsal_activation(1, self.ms.get_dorsal_activation(0) / 2 * 3)
            #     self.body.set_ventral_activation(1, self.ms.get_ventral_activation(0) / 2 * 1.8)
            #     # other segments
            #     for i in range(2, (self.segment_num - 2)):
            #         mi = int((i - 2) / 2)
            #         # print("s_i:",i," mi:",mi,"mi+1:",mi+1)
            #         # print(self.ms.get_dorsal_activation(mi),self.ms.get_ventral_activation(mi))
            #         self.body.set_dorsal_activation(i, (
            #                 self.ms.get_dorsal_activation(mi) + self.ms.get_dorsal_activation(mi + 1)) / 2 * 1.8)
            #         self.body.set_ventral_activation(i, (
            #                 self.ms.get_ventral_activation(mi) + self.ms.get_ventral_activation(mi + 1)) / 2 * 1.8)
            #         # Last two segments
            #     self.body.set_dorsal_activation(self.segment_num - 2,
            #                                     self.ms.get_dorsal_activation(self.muscle_num - 1) / 2 * 1.8)
            #     self.body.set_ventral_activation(self.segment_num - 2,
            #                                      self.ms.get_ventral_activation(self.muscle_num - 1) / 2 * 1.8)
            #     self.body.set_dorsal_activation(self.segment_num - 1,
            #                                     self.ms.get_dorsal_activation(self.muscle_num - 1) / 2 * 1.8)
            #     self.body.set_ventral_activation(self.segment_num - 1,
            #                                      self.ms.get_ventral_activation(self.muscle_num - 1) / 2 * 1.8)
            ...
            self.time_step += 1
            #print("self.time",self.time_step)
    ...
    def set_muscles_input_b(self, neuron_net):
        ...
        for i in range(vncb_start, self.muscle_num):
            ...
            self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * dorsalInput)
            ventralInput = self.parameters["vncb_ms_D_gain"] * vncb_output[neuron_net.get_vncb_id("VD", unit_index=mi)] + \
