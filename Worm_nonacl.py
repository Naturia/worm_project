...
class Worm_n:
    def __init__(self, segment_num=50, muscle_num=24, step_size=0.01, parameters=None, test=False, print_info=True,
                 shared_lists=None, gui_parameters=None, nacl_location=None, state_queue=None):

        if parameters is None:
            self.parameters = {
                "head_ms_SMD_gain": 0.27246094,  # 0.4
                "head_ms_RMD_gain": 0.69335938,  # 0.6
                "tail_ms_SMD_gain": 0.27246094,  # 0.4
                "tail_ms_RMD_gain": 0.69335938,  # 0.6
                "vnc_ms_D_gain": -0.1,
                "vnc_ms_B_gain": 0.6,
                "vnc_ms_A_gain": 0.6,
                "vncb_ms_D_gain": -0.1,
        ...
        self.sr = StretchReceptor(self.segment_num, 7, self.parameters["vnc_sr_gain"],
                                  self.parameters["head_sr_gain"])  # numseg #numsr #srvncgain, srheadgain
        ...
        # self.touch_signal = touch_signal
    ...
    def step(self, neuron_net, nacl, touch):
        # update stretch recepctor
        head_muscles_num = 6
        muscle_num = 24

        self.avm_h = neuron_net.AVM.get_v()
        self.plm_h = neuron_net.PLM.get_v()

        print("self.avm_h", self.avm_h, "self.plm_h", self.plm_h)

        if self.avm_h == 0 and self.plm_h == 0:
            ...
            if self.test:
                ...
                self.neuron_net_update(neuron_net, nacl, touch)
                # set muscle input
            ...
            for i in range(2, (self.segment_num - 2)):
                ...
                self.body.set_dorsal_activation(i, (
                        self.ms.get_dorsal_activation(mi) + self.ms.get_dorsal_activation(mi + 1)) / 2)
                self.body.set_ventral_activation(i, (
                        self.ms.get_ventral_activation(mi) + self.ms.get_ventral_activation(mi + 1)) / 2)
                # Last two segments
            self.body.set_dorsal_activation(self.segment_num - 2,
                                            self.ms.get_dorsal_activation(self.muscle_num - 1) / 2)
            self.body.set_ventral_activation(self.segment_num - 2,
                                             self.ms.get_ventral_activation(self.muscle_num - 1) / 2)
            self.body.set_dorsal_activation(self.segment_num - 1,
                                            self.ms.get_dorsal_activation(self.muscle_num - 1) / 2)
            self.body.set_ventral_activation(self.segment_num - 1,
                                             self.ms.get_ventral_activation(self.muscle_num - 1) / 2)
        ...
        elif self.avm_h >= 0.01 or self.plm_h >= 0.01:
            for i in range(self.segment_num):
                d = (self.body.get_dorsal_len(i) - self.body.get_rest_len(i)) / self.body.get_rest_len(i)
                v = (self.body.get_ventral_len(i) - self.body.get_rest_len(i)) / self.body.get_rest_len(i)
                # print(i,d,v)
                self.sr.update_dorsal_input(self.segment_num - i - 1, d)
                self.sr.update_ventral_input(self.segment_num - i - 1, v)
            self.sr.step()
            ...
            if self.test:
                ...
                # dorsalHeadInput = self.parameters["head_ms_SMD_gain"] * head_output[neuron_net.get_head_id("SMDD")] + \
                #                   self.parameters["head_ms_RMD_gain"] * head_output[neuron_net.get_head_id("RMDD")]
                # ventralHeadInput = self.parameters["head_ms_SMD_gain"] * head_output[neuron_net.get_head_id("SMDV")] + \
                #                    self.parameters["head_ms_RMD_gain"] * head_output[neuron_net.get_head_id("RMDV")]
                # for i in range(head_muscles_num):
                #     # print(i,self.nmj_gain[i-1],dorsalHeadInput,ventralHeadInput)
                #     self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * dorsalHeadInput)
                #     self.ms.update_ventral_input(muscle_num - i - 1, self.nmj_gain[i] * ventralHeadInput)
                #     # print("ms:",self.ms.get_dorsal_activation(i),self.ms.get_ventral_activation(i)) #activation between 0 and 1
                #
                # # VNC
                ...
                #     # print("m_i:",i," vi:",mi)
                #     dorsalInput = self.parameters["vnc_ms_D_gain"] * vnc_output[
                #         neuron_net.get_vnc_id("DD", unit_index=mi)] + \
                #                   self.parameters["vnc_ms_A_gain"] * vnc_output[
                #                       neuron_net.get_vnc_id("DA", unit_index=mi)]
                #     self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * dorsalInput)
                #     ventralInput = self.parameters["vnc_ms_D_gain"] * vnc_output[
                #         neuron_net.get_vnc_id("VD", unit_index=mi)] + \
                #                    self.parameters["vnc_ms_A_gain"] * vnc_output[
                #                        neuron_net.get_vnc_id("VA", unit_index=mi)]
                #     self.ms.update_ventral_input(muscle_num - i - 1, self.nmj_gain[i] * ventralInput)
                # self.ms.step(self.step_size)
                self.neuron_net_update_b(neuron_net, nacl, touch)
                # set muscle input
                self.set_muscles_input_b(neuron_net)
                self.ms.step(self.step_size)
            # update body
            # first two segments
            self.body.set_dorsal_activation(0, self.ms.get_dorsal_activation(0) / 2)
    ...
    def set_muscles_input(self, neuron_net):
        ...
        for i in range(vnc_start, self.muscle_num):
            mi = int((i - vnc_start) / muscle_per_unit)
            # print("m_i:",i," vi:",mi)
            dorsalInput = self.parameters["vnc_ms_D_gain"] * vnc_output[neuron_net.get_vnc_id("DD", unit_index=mi)] + \
                          self.parameters["vnc_ms_B_gain"] * vnc_output[neuron_net.get_vnc_id("DB", unit_index=mi)]
            self.ms.update_dorsal_input(i, self.nmj_gain[i] * dorsalInput)
            ventralInput = self.parameters["vnc_ms_D_gain"] * vnc_output[neuron_net.get_vnc_id("VD", unit_index=mi)] + \
                           self.parameters["vnc_ms_B_gain"] * vnc_output[neuron_net.get_vnc_id("VB", unit_index=mi)]
    ...
    def set_muscles_input_b(self, neuron_net):
        tail_muscles_num = 6
        muscle_num = 24
        tail_output = neuron_net.get_all_tail_neuron_out()
        dorsaltailInput = self.parameters["head_ms_SMD_gain"] * tail_output[neuron_net.get_tail_id("SMDDB")] + \
                          self.parameters["head_ms_RMD_gain"] * tail_output[neuron_net.get_tail_id("RMDDB")]
        ventraltailInput = self.parameters["head_ms_SMD_gain"] * tail_output[neuron_net.get_tail_id("SMDVB")] + \
                           self.parameters["head_ms_RMD_gain"] * tail_output[neuron_net.get_tail_id("RMDVB")]
        for i in range(tail_muscles_num):
            # print(i,self.nmj_gain[i-1],dorsalHeadInput,ventralHeadInput)
            self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * dorsaltailInput)
            self.ms.update_ventral_input(muscle_num - i - 1, self.nmj_gain[i] * ventraltailInput)
            # print("ms:",self.ms.get_dorsal_activation(i),self.ms.get_ventral_activation(i)) #activation between 0 and 1

        # VNC
        vncb_output = neuron_net.get_all_vncb_neuron_out()
        vncb_start = tail_muscles_num
        muscle_per_unit = 3
        for i in range(vncb_start, self.muscle_num):
            mi = int((i - vncb_start) / muscle_per_unit)
            # print("m_i:",i," vi:",mi)
            dorsalInput = self.parameters["vncb_ms_D_gain"] * vncb_output[neuron_net.get_vncb_id("DD", unit_index=mi)] + \
                          self.parameters["vncb_ms_A_gain"] * vncb_output[neuron_net.get_vncb_id("DA", unit_index=mi)]
            self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * dorsalInput)
            ventralInput = self.parameters["vncb_ms_D_gain"] * vncb_output[
                neuron_net.get_vncb_id("VD", unit_index=mi)] + \
                           self.parameters["vncb_ms_A_gain"] * vncb_output[neuron_net.get_vncb_id("VA", unit_index=mi)]
            self.ms.update_ventral_input(muscle_num - i - 1, self.nmj_gain[i] * ventralInput)
            # print(i,dorsalInput,ventralInput)
        ...
        # if self.time > self.adjust_time:
        #     ventral_klinotaxis, dorsal_klinotaxis = neuron_net.get_klinotaxis_output()
        #     self.ventral_klinotaxis_h, self.dorsal_klinotaxis_h, self.time_h = self.flip(neuron_net,
        #                                                                                  self.ventral_klinotaxis_h,
        #                                                                                  self.dorsal_klinotaxis_h,
        #                                                                                  self.time_h)
        #     ventral_klinotaxis, dorsal_klinotaxis = self.flip_helper(ventral_klinotaxis, dorsal_klinotaxis,
        #                                                              self.ventral_klinotaxis_h,
        #                                                              self.dorsal_klinotaxis_h)
        #     self.avm_h = neuron_net.AVM.get_v()
        #     # print(ventral_klinotaxis,dorsal_klinotaxis)
        ...
        # #delete
        #
        # self.ventral_klinotaxis_trace, self.dorsal_klinotaxis_trace = ventralHeadInput * (
        #             1 - ventral_klinotaxis), dorsalHeadInput * (1 - dorsal_klinotaxis)
        #
        # for i in range(head_muscles_num):
        #     if self.time > self.adjust_time:
        #         self.ms.update_dorsal_input(muscle_num - i -1, self.nmj_gain[i] * (
        #                     dorsalHeadInput + dorsalHeadInput * (1 - dorsal_klinotaxis) * self.parameters["turn_gain"]))
        #         self.ms.update_ventral_input(muscle_num - i -1, self.nmj_gain[i] * (
        #                     ventralHeadInput + ventralHeadInput * (1 - ventral_klinotaxis) * self.parameters[
        #                 "turn_gain"]))
        #     else:
        #         self.ms.update_dorsal_input(muscle_num - i -1, self.nmj_gain[i] * dorsalHeadInput)
        #         self.ms.update_ventral_input(muscle_num - i -1, self.nmj_gain[i] * ventralHeadInput)
        ...
        # # VNC
        # vncb_output = neuron_net.get_all_vncb_neuron_out()
        # vncb_start = head_muscles_num
        # muscle_per_unit = 3
        #
        # for i in range(vncb_start, self.muscle_num):
        #     mi = int((i - vncb_start) / muscle_per_unit)
        #     # print("m_i:",i," vi:",mi)
        #     dorsalInput = self.parameters["vnc_ms_D_gain"] * vncb_output[neuron_net.get_vncb_id("DD", unit_index=mi)] + \
        #                   self.parameters["vnc_ms_A_gain"] * vncb_output[neuron_net.get_vncb_id("DA", unit_index=mi)]
        #     ventralInput = self.parameters["vnc_ms_D_gain"] * vncb_output[neuron_net.get_vncb_id("VD", unit_index=mi)] + \
        #                    self.parameters["vnc_ms_A_gain"] * vncb_output[neuron_net.get_vncb_id("VA", unit_index=mi)]
        #     if self.time > self.adjust_time:
        #         self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * (
        #                     dorsalInput + dorsalInput * (1 - dorsal_klinotaxis) * 0.5))
        #         self.ms.update_ventral_input(muscle_num - i - 1, self.nmj_gain[i] * (
        #                     ventralInput + ventralInput * (1 - ventral_klinotaxis) * 0.5))
        #     else:
        #         self.ms.update_dorsal_input(muscle_num - i - 1, self.nmj_gain[i] * dorsalInput)
        #         self.ms.update_ventral_input(muscle_num - i - 1, self.nmj_gain[i] * ventralInput)
        #     # print(i,dorsalInput,ventralInput)
    ...
    def flip(self, neuron_net, ventral_klinotaxis_h, dorsal_klinotaxis_h, time_h):
        ...
        if (neuron_net.ASER.get_v() - self.aser_h) > self.parameters["concentration_th"] and (self.time - time_h) > \
                self.parameters["min_turn_time"]:
    ...
    def neuron_net_update(self, neuron_net, nacl, touch):
        # nacl
        ...
        # touch
        if touch[0] == 1:
            neuron_net.AVM.step(100)
            touch[0] = 0  # reset touch signal
        elif touch[0] == 0:
            neuron_net.AVM.step(0)

        if touch[1] == 1:
            neuron_net.PLM.step(100)
            touch[1] = 0  # reset touch signal
        elif touch[1] == 0:
            neuron_net.PLM.step(0)
        ...
        #     neuron_net.set_vnc_input(self.sr.get_vnc_ventral(i), self.sr.get_vnc_dorsal(i), i)
        #     # if i >= 0:
        #     # self.collect_for_test.append([self.sr.get_vnc_ventral(i),self.sr.get_vnc_dorsal(i)])
        #     # print("VNC:",i,self.sr.get_vnc_ventral(i),self.sr.get_vnc_dorsal(i))
        # neuron_net.run()
    ...
    def neuron_net_update_b(self, neuron_net, nacl, touch):
        # nacl
        worm_x = self.body.get_center_x()
        worm_y = self.body.get_center_y()
        # concentration = nacl.get_concentration(worm_x, worm_y)
        ...
        # tail
        tail_inputs = np.zeros(len(neuron_net.tailneuron_index))
        if not self.pause:
            tail_inputs[neuron_net.tailneuron_index['SMDDB']] = self.sr.get_head_dorsal()
            tail_inputs[neuron_net.tailneuron_index['SMDVB']] = self.sr.get_head_ventral()
        neuron_net.set_tail_input_faster(tail_inputs)
        ...
        vncb_inputs = np.zeros(neuron_net.unit_num * neuron_net.neuron_per_unit + 1)
        if not self.pause:
            da_index = neuron_net.vncbneuron_index['DA']
            va_index = neuron_net.vncbneuron_index['VA']
            for i in range(neuron_net.unit_num):
                unit_jumper = i * neuron_net.neuron_per_unit
                # print(unit_jumper + db_index,unit_jumper + vb_index)
                vncb_inputs[unit_jumper + da_index] = self.sr.get_vnc_ventral(i)
                vncb_inputs[unit_jumper + va_index] = self.sr.get_vnc_dorsal(i)
        neuron_net.set_vncb_input_faster(vncb_inputs)
    ...
    def get_state(self, worm_net):
        ...
        # touch
        state_dict["AVM"] = worm_net.AVM.get_v()
        state_dict["PLM"] = worm_net.PLM.get_v()
